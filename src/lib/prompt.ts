...
- {"kind":"js_event_loop","sayIntro":"optional","title":"<=60","steps":[{"op":"push|pop|enqueue|drainMicro|takeMacro|render","label":"<=22 frame or callback e.g. \"main()\", \"log('C')\"","queue":"micro|macro — REQUIRED for enqueue","via":"<=16 what parked it: setTimeout, fetch, Promise, rAF","note":"<=44 optional caption","say"}, 2-16]} — THE JavaScript/browser event loop as a machine: a call stack, a microtask queue and a macrotask queue side by side, and the rule that connects them. "enqueue" drops a callback into one of the two queues (a chip that flies in from the web-API lane), "drainMicro" empties the WHOLE microtask queue one chip at a time, "takeMacro" moves exactly ONE macrotask, "render" is the paint step. A "takeMacro" issued while the microtask queue is non-empty is REFUSED on screen with a "microtasks first" badge — author that refusal deliberately, it is the lesson. Use this for why Promise.then beats setTimeout(0), what a microtask actually is, microtask starvation, and why a long callback freezes the page. NOT for asyncio/coroutines/await — that is "event_loop", which has a ready ring and no queues at all.
