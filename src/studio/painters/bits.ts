...
export function paintBits(ctx: CanvasRenderingContext2D, scene: BitsScene, env: PaintEnv) {
  ...
  // Cells are a PIXEL row, drawn directly — no camera round-trip needed.
  ...
  const shimmerAt = (i: number) => {
    const w = Math.sin(env.elapsedMs / 1200 - i * 0.55);
    return Math.max(0, w) * Math.max(0, w);
  };

  // Per-cell fill state, computed directly rather than through a frozen 3D
  // closure — same bit/flash/scale logic the removed material carried.
  for (let i = 0; i < width; i++) {
    ...
    if (ghost) {
      ...
      if (s <= 0) {
        ...
        visible = false; // moving digits are drawn separately below
      }
    } else {
      bit = next[i];
      flash = shimmerAt(i) * 0.5;
    ...
    if (!visible) continue;
    const r = cellRect(i);
    const cellAlpha = frameIn * leave * (ghost ? 0.4 : 0.9);

    ctx.save();
    ctx.globalAlpha = cellAlpha;
    ctx.translate(r.cx, r.cy);
    ctx.scale(Math.max(0.001, scale * flipAmount), Math.max(0.001, scale));
    ctx.translate(-r.cx, -r.cy);

    const face =
      flash > 0
        ? lerpColor(THEME.panel, accent, SET_TINT + 0.3 * flash)
        : ghost
          ? shade(THEME.panel, -0.2)
          : bit
            ? lerpColor(THEME.panel, accent, SET_TINT)
            : shade(THEME.panel, IDLE_FACE_LIFT);

    applyElevation(ctx, unit, flash > 0 ? "floating" : "raised");
    if (flash > 0) {
  ...
  for (let i = 0; i < width; i++) {
    ...
    if (!hideDigit) {
      ctx.save();
      ctx.globalAlpha = frameIn * leave * (ghost ? 0.5 : 1.0);
  ...
  if (isShift) {
    ...
    if (s > 0 && s < 1) {
      ...
      for (let i = 0; i < width; i++) {
        ...
        ctx.globalAlpha = (falling ? 1 - s : 1) * frameIn * leave;
        ctx.font = `700 ${unit * 1.4}px ${FONT_MONO}`;
      ...
      const inFrom =
        op === "shl"
          ? get2D(width - 1).x + Math.abs(get2D(1).x - get2D(0).x) * (1 - s)
          : get2D(0).x - Math.abs(get2D(1).x - get2D(0).x) * (1 - s);
      ctx.globalAlpha = s * frameIn * leave;
      ctx.font = `700 ${unit * 1.4}px ${FONT_MONO}`;
    ...
    if (arrowAlpha > 0) {
      const ay2 = get2D(0).y - unit * 2.5;
      const spanCx = (get2D(0).x + get2D(width - 1).x) / 2;
      const half = Math.min(Math.abs(get2D(width - 1).x - get2D(0).x) * 0.22, unit * 3);
      ctx.save();
      ctx.globalAlpha = arrowAlpha * 0.55 * frameIn * leave;
      ctx.strokeStyle = accent;
      ctx.fillStyle = accent;
      ctx.lineWidth = unit * 0.1;
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(spanCx - half * dir, ay2);
      ctx.lineTo(spanCx + half * dir, ay2);
      ctx.stroke();
      drawArrowhead(ctx, spanCx + half * dir, ay2, dir > 0 ? 0 : Math.PI, unit * 0.5);
      ctx.restore();
    }
  }
  ...
  if (isLogic) {
    ...
    ctx.save();
    ctx.globalAlpha = opIn * fade * frameIn * leave;
    for (let i = 0; i < width; i++) {
      const ox = get2D(i).x;
  ...
  const drawReadout = (dec: number, alpha: number, dy: number) => {
    ...
    const text = `= ${dec}`;
    ctx.save();
    ctx.globalAlpha = alpha * frameIn * leave;
  ...
  if (step?.note) {
    ...
    if (noteAlpha > 0) {
      ...
      ctx.save();
      ctx.globalAlpha = noteAlpha * frameIn * leave;
