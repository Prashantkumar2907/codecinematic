...
| 7.1 | 7 | render | Re-lock 7 desynced painters to beat windows | done | 0.1 | all 7 now use `activeBeatIndex(env.beats, …)` instead of `Math.floor(env.p * n)`; 0 desync sites remain, `tsc` unchanged at 71. **Not yet `verified`** — filmstrip capture is blocked by 7.3 | 652cd27 |
| 7.2 | 7 | render | Progressive reveal for the 5 single-beat painters | todo | 4.1 | no dead frame at cap — **not started** | — |
| 7.3 | 7 | tooling | **BLOCKER — `/probe` harness no longer initialises** | blocked | — | page returns 200 but `window.__PROBE_DONE` stays `null` and `__PROBE_KINDS` is empty, so `filmstrip`/`edge-audit` both time out. Reproduced with all Phase 7 painter edits stashed, so it is **not** caused by them. Unblocks 7.1 verification, 9.x and 10.x | — |
