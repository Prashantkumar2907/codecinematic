...
| 4.3 | 4 | content | ~~Soft gate `definitionOpener`~~ | wont-do | 3.2 | **the problem does not exist**: 3 of 88 by a real definition pattern. The plan's pattern flags 25 but 21 are good `Your…` cold-opens, so gating on it would degrade the writing. Both predicates stay in `pacing.ts`; the audit reports both. See 3.3 | ff63c0b |
